...
export const handleSignup = AsyncHandler(async (req, res) => {
  ...
  const sessionTokens = await createSession(newUser.id, clientIp, userAgent);
...
export const handleSignin = AsyncHandler(async (req, res) => {
  ...
  const tokens = await createSession(user.id, clientIp, userAgent);
...
export const refreshAccessToken = AsyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (!secretKey) {
      throw new ApiError(
        500,
        'Server misconfiguration: Refresh token secret is missing.'
      );
    }
    ...
    const tokens = await createSession(decoded.sub, clientIp, userAgent);
